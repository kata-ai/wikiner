...
from allennlp.modules import ConditionalRandomField, Embedding
...
@Model.register("crf_tagger_crosslingual")
class CrfTaggerCrossLingual(Model):
    """
    ...
    lang_namespace : ``str``, optional (default=``lang_id``)
        This is needed to add language id to alongside word features.
        Unless you did something unusual, the default value should be what you want.
    lang_dim : ``int``, optional (default=``10``)
        Dimension of language embedding features
    ...
    def forward(self,  # type: ignore
        ...
        tokens : ``Dict[str, torch.LongTensor]``, required
            ...
            token in your input.
        lang_indicator: torch.LongTensor, required.
        ...
        embedded_lang_input = self.lang_embedding(lang_indicator.long())
        embedded_text_with_lang_indicator = torch.cat([embedded_text_input, embedded_lang_input], -1)

        if self.dropout:
            # embedded_text_input = self.dropout(embedded_text_input)
            embedded_text_with_lang_indicator = self.dropout(embedded_text_with_lang_indicator)

        # encoded_text = self.encoder(embedded_text_input, mask)
        encoded_text = self.encoder(embedded_text_with_lang_indicator, mask)
    ...
    @classmethod
    def from_params(cls, vocab: Vocabulary, params: Params) -> 'CrfTaggerCrossLingual':
        embedder_params = params.pop("text_field_embedder")
        text_field_embedder = TextFieldEmbedder.from_params(vocab, embedder_params)
        encoder = Seq2SeqEncoder.from_params(params.pop("encoder"))
        label_namespace = params.pop("label_namespace", "labels")
        # lang_namespace
        lang_map = params.pop("lang_map")
        # lang_dimension
        lang_dim = params.pop("lang_dim")
